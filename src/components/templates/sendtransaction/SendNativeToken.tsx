...
import { INativeBalances } from './types';

const SendNativeToken: FC<INativeBalances> = ({ balance }) => {
    ...
    <>
      {address ?
        <form onSubmit={
          ...
          <Center>
            <Text>Send {nativeTokenSymbol}</Text>
          </Center>
          <Text>Balance: {(Number(balance.balance) / 1e18).toFixed(3)} {nativeTokenSymbol}</Text>
          <FormControl isRequired>
            <FormLabel htmlFor='reciever'>To</FormLabel>
            <Input
              id='reciever'
