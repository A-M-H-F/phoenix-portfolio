...
import { ILotteryWeek } from './types';
import {
  Box,
  Button,
  Center,
...
const LotteryWeek: FC<ILotteryWeek> = ({ balance }: any) => {
  const { chain } = useNetwork();
  const { address } = useAccount();
  const toast = useToast();
  const contractAddress = process.env.NEXT_PUBLIC_LOTTERY_WEEK;
